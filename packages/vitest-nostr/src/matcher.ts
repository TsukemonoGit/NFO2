...
import { deriveToReceiveMessage as deriveToReceiveMessageBase } from "vitest-websocket-mock";
...
function matchEvent(
  ...
  const isEvent = (x: unknown): x is Nostr.Event =>
    typeof x === "object" &&
    x !== null &&
    eventKeys.every((key) => key in x);

  return (
    isEvent(received) &&
    eventKeys.every(
      (key) => expected[key] === undefined || equal(received[key], expected[key]),
    )
  );
}
  ...
  const subId =
    typeof pred === "string"
      ...
      : pred !== undefined
        ...
        : null;
  const count =
    typeof pred === "object" && pred !== undefined ? pred[1] : null;
...
const deriveToReceiveMessage = deriveToReceiveMessageBase;

function clientMathcer(
  name: string,
  fn: RawMatcherFn,
): RawMatcherFn {
  return async function (spy: ClientSpy, pred: unknown, options?: unknown) {
    if ((spy as ClientSpy & { __createClientSpy?: boolean }).__createClientSpy) {
      try {
        return await withTimeout(
          async () => fn.call(this, await spy.next(), pred, options),
        );
      } catch (err) {
      ...
      return {
        ...
        message: () =>
          ...
          "\n\n" +
          `Matcher \`${name}\` must be used for ClientSpy, but now being used for:\n\n` +
